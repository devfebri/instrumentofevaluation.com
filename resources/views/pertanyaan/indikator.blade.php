            ...
            <form class="needs-validation" id="form-tambah-edit" name="form-tambah-edit">
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        <h6 class="text-muted fw-400 mt-3">Indikator</h6>
                        <input type="text" class="form-control" name="indikator" id="indikator" required>
                        <input type="hidden" class="form-control" name="mindset_id" id="mindset_id" value="{{ $id }}" >
                    </div>
                </div>
...
<script>
    ...
    $(document).ready(function() {
        var table = $('#datatable1').DataTable({
            processing: true
            , serverSide: true
            , ajax: "{{ route(auth()->user()->role.'_openmindset',$id) }}"
            , columns: [{
                    data: null
                    , sortable: false
        ...
        $('#tombol-tambah').click(function() {
            ...
            $('#modal-judul').html("Tambah Indikator"); //valuenya tambah pegawai baru
            ...
            $("#form-tambah-edit").validate({
                submitHandler: function(form) {
                    ...
                    $.ajax({
                        ...
                        url: "{{ route('admin_indikatortambah') }}", //url simpan data
        ...
        $('body').on('click', '.delete', function(id) {
            ...
            var url = "{{ route(auth()->user()->role.'_indikatordelete', ':dataid') }}";

            urls = url.replace(':dataid', dataid);
            alertify.confirm('Seluruh soal yang ada pada indikator ini akan ikut terhapus, apa anda yakin ?', function() {
                $.ajax({
